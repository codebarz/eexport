...
<head>
    <title>Background Check | Welcome</title>
    <meta charset="utf-8">
...
<input type="search" onkeydown="searchq()" name="hssearch" class="hssearch" id="hssearch" placeholder="Search HS Codes">
</form>
</li>
<li class="barheading">Menu</li>
<?php echo'<a href="search.php?userid='.$id.'"><li>Export Search Engine</li></a>';?>
<?php echo '<a href="backgroundcheck.php?userid='.$id.'"><li class="tabactive_2">Background Checks</li></a>';?>
...
<div class="container">
  <?php
    ...
    if((!empty($get)) && ($get == 1))
    {
        echo "<div class='alertSuccess'>Request has been successfully sent we would get back to you shortly.</div>";
    }
    ?>
  <?php
        ...
        if((!empty($get)) && ($get == 1))
        {
            echo "<div class='alertFailure'>There was an error sending your request. Please try again.</div>";
        }
        ?>
<div class="loadingArea_2">
  <div class="post_req">
      ...
      <form action="checks.php" method="post" enctype="multipart/form-data">
          <textarea name="check_req" id="check_req" placeholder="example: I would like a background check on Ace Welding Co., 42, Lagos Rd. on how good they are with export"
                    rows="6"></textarea><br>
