...
<html>
    ...
    <div>      
			<form name="frmSearch" method="post" action="asearch.php">
				...
				<div id="advanced-search-box" <?php if(empty($advance_search_submit)) { ?>style="display:none;"<?php } ?>>
					...
					<div>
						<select name="search[search_in]" id="search_in" class="demoInputBox">
							...
							<option value="question" <?php if($search_in=="question") { echo "selected"; } ?>>Question</option>
							<option value="answer" <?php if($search_in=="answer") { echo "selected"; } ?>>Answer</option>
						</select>
					</div>
				</div>
			...
			<div>
				<div><strong><?php echo $row["question"]; ?></strong></div>
				<div class="result-description"><?php echo $row["answer"]; ?></div>
			</div>
			<?php } ?>
		</div>
	</body>
