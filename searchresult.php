...
<body>
  ...
  <ul>
  <li><?php echo '<a href="searchresult.php?mainSearch='.$_GET['mainSearch'].'">Main</a>';?></li>
  <li>Images</li>
  <li>Videos</li>
  <a href="quicksearch.php"><li title="Search faster with exact word" class="special">Qucik Search</li></a>
  </ul>
  </div>
  </div>
 ...
 if (isset($_GET["mainSearch"])) 
    {
      ...
      if ($resultCount > 0)
      {
      if ($result && $mainAnsRet)
      {
          while ($mainRow = $mainAnsRet->fetchArray(SQLITE3_ASSOC))
          {
              $question = $mainRow['question'];
            $answer = $mainRow['answer'];
            $cid = $mainRow['category_id'];

            echo '<div class="quesbox_3">
                <div class="questitle">
                    <h2>'.$mainRow["question"].'</h2>
                </div>
                <div class="quesanswer">'.$mainRow["answer"].'</div>
            </div>';
          }
